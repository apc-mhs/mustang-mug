...
async function getDocumentsWhere(collection, queryOnQueryable) {
    ...
    const [modified, cached] = await Promise.all([
        queryOnQueryable(app.firestore().collection(collection).where('lastModified', '>', getLastModified(collection))).get(),
        queryOnQueryable(app.firestore().collection(collection)).get({ source: 'cache' })
    ]).then((snapshots) => {
